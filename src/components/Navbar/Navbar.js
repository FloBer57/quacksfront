...
const Navbar = ({ person }) => {
  ...
  useEffect(() => {
    const status = statusList.find(
      (status) => status.PersonStatut_Id === person.personStatut_Id
    );
  ...
  const handleStatusChange = async (status) => {
    try {
      ...
      await updatePerson(person.person_Id, {
        StatutId: status.PersonStatut_Id,
      });
      setCurrentStatus(status);
      console.log(
        "Statut mis à jour avec succès vers :",
          ...
          <ul className="flex-row profilbar">
            ...
            <li className="nav-item dropdown me-3 me-lg-1">
              <Dropdown>
                ...
                <Dropdown.Menu className="dropdown-menu">
                  <Dropdown.Item onClick={() => handleNavigate("/UserPage")}>
                    Mon compte
                  </Dropdown.Item>
                  {person.personRole_Id === 2 ? (
                    <Dropdown.Item onClick={() => handleNavigate("/AdminPage")}>
                      Administrateur
                    </Dropdown.Item>
                  ) : null}
                ...
                <Card.Body>
                  ...
                  <Card.Footer>
                    <small className="text-muted">
                      {new Date(
                        notification.notification_DatePost
                      ).toLocaleString()}
                    </small>
                    <Button
                      variant="danger btn-notification"
                      size="sm"
                      onClick={() =>
